...
The default settings in __gbm__ includes a learning rate (`shrinkage`) of 0.001. This is a very small learning rate and typically requires a large number of trees to find the minimum MSE.  However, __gbm__ uses a default number of trees of 100, which is rarely sufficient.  Consequently, if you use the default learning rate you will likely want to significantly increase the number of trees applied.  The default depth of each tree (`interaction.depth`) is 1, which means we are ensembling a bunch of stumps. Lastly, I also include `cv.folds` to perform a 5 fold cross validation.  
...
```{r gbm1-gradient-descent, echo=FALSE, fig.cap="Training and cross-validated MSE as *n* trees are added to the GBM algorithm.", out.height="90%", out.width="90%"}
...
As we've seen in the previous chapters, rarely do the default settings suffice.  We could tune parameters one at a time to see how the results change.  For example, here, I increase the learning rate to take larger steps down the gradient descent, keep the number of trees the same (maybe moving down the gradient descent faster will allow us to find a minimum error with 5000 or less trees), and increase the depth of each tree from using a single split to 3 splits. Our RMSE (\$23,813.34) is lower than our initial model and the optimal number of trees required was 964.
...
```{r gbm2-gradient-descent, echo=FALSE, fig.cap="Training and cross-validated MSE as *n* trees are added to the GBM algorithm. We can see that are new hyperparameter settings result in a much quicker progression down the gradient descent than our initial model.", out.height="90%", out.width="90%"}
...
The grid search performed next reveals a few important attributes.  First, our top model has better performance than the models fit in previous chapters, with an RMSE of \$22,424. Second, looking at the top 10 models (`gbm_tune$results` or as illustrated in Figure \@ref(fig:gbm-tune-results)) you will see that:

- all the top models used a learning rate of 0.1 or smaller; small incremental steps down the gradient descent appear to work best,
...
```{r gbm-tune-results, echo=FALSE, fig.cap="Model performance of our GBM grid search.  Smaller learning rate, greater tree depth, and terminal nodes with smaller number of observations all appear to help minimize error.", out.height="95%", out.width="95%"}
...
You may also want to assess the impact of including a stochastic gradient descent with `bag.fraction`.  Also, since this grid search would use a smaller learning rate, you may want to increase the number of trees to ensure you are converging to a minimal error.
...
1. `method = relative.influence`: At each split in each tree, `gbm` computes the improvement in the split-criterion (MSE for regression, Gini index for classification). `gbm` then averages the improvement made by each variable across all the trees that the variable is used. The variables with the largest average decrease in the loss function are considered most important.
...
Looking at the other two top influential variables we see that as the quality of the home increases, in general we see an increase in sale price.  However, closer inspection of Figure \@ref(fig:overall-qual-gbm) shows several observations that have a decrease in sales price as the quality moves from very poor to average and then they increase beyond that.  We also see two diverging bands that have noticable differences in their home sales price when the quality improves to very good and higher.  This suggests that there may be some interaction effects happening behind the scenes.

```{r overall-qual-gbm, echo=FALSE, fig.width=10, fig.cap="Centered ICE curves for the Overall Quality variable indicates that increasing a homes quality from very poor to very excellent can increase the predicted sales price by over $50,000 on average."}
gbm_final_fit %>%
...
Also, looking at the PDP plot for the Neighborhood variable indicates that the Greens neighborhood demands a substantially larger predicted sales price than all other neigbhorhoods. 

```{r, echo=FALSE, fig.height=6, fig.cap="PDP plot for the Neighborhood variable indicates that the Greens neighborhood demands a substantially larger predicted sales price than all other neigbhorhoods."}
gbm_final_fit %>%
...
We can use this information to create a partial dependence map that helps to visualize the average predicted sales prices based on geographic location.  
...
```{r, echo=FALSE, out.height="80%", out.width="80%", fig.cap="Partial dependence map that clearly shows the geographic location of neighbhorhoods that drive predicted sales prices in our GBM model."}
knitr::include_graphics("illustrations/partial-dependence-map.png")
```
...
With the Ames data, the GBM model obtained the best predictive accuracy thus far, but how about the employee attrition data?  The following performs a grid search across 54 hyperparameter combinations.  
...
Our best GBM model provides an accuracy of 87.5%, which provides a slight improvement over the previous best model which was an Elast net.  

```{block, type = "warning"}
One thing to consider when your improvement is this small, does the additional time and computational resources required for a GBM model warrant the 0.002 improvement in accuracy?
```
...
To learn more I would start with the following resources:
