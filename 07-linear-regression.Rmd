...
```{r 07-fm1}
fm1 <- lm(Sale_Price ~ Gr_Liv_Area, data = trn)
```

The fitted model (`fm1`) is displayed in the left plot in Figure \@ref(fig:07-visualize-fm1) where the points represent the values of `Sale_Price` in the training data. In the right plot of Figure  \@ref(fig:07-visualize-fm1), the vertical lines represent the individual errors, called _residuals_, associated with each observation. The OLS criterion \@ref(eq:rss) identifies the "best fitting" line that minimizes the sum of squares of these residuals.

```{r 07-visualize-fm1, eval=TRUE, fig.width=10, fig.height=3.5, echo=FALSE, fig.cap="The least squares fit from regressing `Sale_Price` on `Gr_Liv_Area` for the the Ames housing data. _Left_: Fitted regresison line. _Right_: Fitted regression line with vertical grey bars representing the residuals."}
...
y_values <- round(trn$Sale_Price, 3)
x1_values <- round(trn$Gr_Liv_Area, 3)
x2_values <- round(trn$Year_Built, 3)
...
df2 <- augment(cv_model3$finalModel, data = trn)
