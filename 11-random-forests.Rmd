...
```{r exemplar-decision-tree, echo=FALSE, fig.cap="Exemplar decision tree predicting if a customer will redeem a coupon (yes or no) based on the customer's loyalty, household income, last month's spend, coupon placement, and shopping mode.", out.height="70%", out.width="70%"}
...
```{r exemplar-decision-tree, echo=FALSE, fig.cap="Terminology of a decision tree.", out.height="70%", out.width="70%"}
knitr::include_graphics("illustrations/decision-tree-terminology.png")
```

### Partitioning

As illustrated above, CART uses binary partitioning. The objective is to assess an individual feature ($x_i$) and split that feature to segment the response variable into one of two regions.  The best binary split is the one in which each subgroup will be as similar or homegenous as possible, while responses across subgroups will be as different or as heterogenous as possible.
...
[^other]: Other decision tree algorithms include the Iterative Dichotomiser 3 [@quinlan1986induction], C4.5 [@quinlan1996bagging], Chi-square automatic interaction detection [@kass1980exploratory], Conditional inference trees [@hothorn2006unbiased], and more. 
